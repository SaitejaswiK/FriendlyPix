...
export default class Uploader {
  ...
  displayVideo(url) {
    this.newVideoContainer.attr('src', URL.createObjectURL(url));
    this.newVideoContainer.css('display', 'block');
    page('/add');
    this.imageCaptionInput.focus();
    this.uploadButton.prop('disabled', true);
  }
  ...
  readPicture(event) {
    ...
    if (file.type.match('video.*')) {
      ...
      this.displayVideo(this.currentFile);
      this.disableUploadUi(false);
    }
  }
 ...
 async uploadVideo(e) {
    ...
    try {
      const postId = await this.firebaseHelper.uploadNewVideo(this.videoFile, this.videoFile.type, this.videoFile.name, imageCaption );
  ...
  clear() {
    ...
    this.newVideoContainer.attr('src', '');
    this.newVideoContainer.css('display', 'none');
