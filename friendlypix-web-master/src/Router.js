...
export default class Router {
  ...
  constructor(auth) {
    ...
    page('/home', () => {
      showHomeFeed(); this.displayPage('feed', true);
    });
    page('/recent', () => {
      showGeneralFeed(); this.displayPage('feed');
    });
    page('/post/:postId', (context) => {
      showPost(context.params.postId); this.displayPage('post');
    });
    page('/user/:userId', (context) => {
      loadUser(context.params.userId); this.displayPage('user-info');
    });
    page('/search/:hashtag', (context) => {
      searchHashtag(context.params.hashtag); this.displayPage('search');
    });
    page('/about', () => {
      clearFeed(); this.displayPage('about');
    });
    page('/terms', () => {
      clearFeed(); this.displayPage('terms');
    });
    page('/add', () => {
      this.displayPage('add', true);
    });
    page('*', () => page('/'));
  ...
  static setLinkAsActive(context, next) {
    let canonicalPath = context.canonicalPath;
    if (canonicalPath === '') {
      canonicalPath = '/';
    }
